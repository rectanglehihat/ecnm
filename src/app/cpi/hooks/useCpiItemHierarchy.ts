...
export const buildParentMap = (items: StatisticItem[]) => {
	...
	console.debug('[useCpiItemHierarchy] buildParentMap', {
		totalItems: items.length,
		rootChildrenCount: rootChildren.length,
		sampleRootCodes: rootChildren.slice(0, 10).map((c) => ({ item: c.ITEM_CODE, norm: (c as any).__normCode })),
	});
...
export const collectLeafCodesForPath = (
	...
	if (path.length > 0) {
		...
		console.debug('[useCpiItemHierarchy] collectLeafCodesForPath', {
			path,
			navSegments,
			leafCount: filtered.length,
			leafSample: filtered.slice(0, 10),
		});
		return filtered;
	}

	const filtered = leaves.filter((c) => c !== startCode);
	console.debug('[useCpiItemHierarchy] collectLeafCodesForPath (no path)', {
		startCode,
		leafCount: filtered.length,
		leafSample: filtered.slice(0, 10),
	});
